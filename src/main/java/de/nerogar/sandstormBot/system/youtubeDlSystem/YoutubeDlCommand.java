...
public class YoutubeDlCommand implements ICommand {
	...
	private List<Song> getSongs(List<String> songLocations) {
		...
		for (String songResponse : sortableSongs) {
			try {
				...
				Song song = new Song(YoutubeDlAudioTrackProvider.NAME, webpage_url, title, artist, sortableSongs.size() > 1 ? query : null, duration, query, member.getEffectiveName());
	...
	private List<String> getSongsInternalSingle(List<String> songLocations) {
		...
		for (Future<SortableSong> sortableSongFuture : sortableSongFutures) {
			try {
				...
				e.getCause().printStackTrace(Main.LOGGER.getWarningStream());
			}
		}

		sortableSongs.sort(Comparator.comparingInt(s -> s.index));
	...
	private class SortableSong {
		...
		private final String songResponse;
		private final int    index;

		public SortableSong(String songLocation, String songResponse, int index) {
